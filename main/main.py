...
while start:
    ...
    if robot.takeBeer == True:    # если груз взят, то везем его
    ...
    elif not robot.deliverBeer:  # если груз не взят и не доставлен
        ...
        if robot.robot_rect.collidepoint(beer.getBeerPos()):  # определяет пересечение поверхности робота и поверхности пива
            robot.takeBeer = True
            beer.takeBeer()  # при взятии пива удаляет его с экрана
